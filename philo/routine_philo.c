...
/*   Updated: 2023/05/21 13:53:21 by fcoindre         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_death(t_philo *philo)
{
	if ((calculate_current_time_ms(philo->start_time) - philo->time_last_eat) > philo->rules->time_to_die)
	{
		...
		{
			philo->is_dead = 1;
			printf("\033[1;3%dm%ld %d died\n\033[0m", philo->id % 7, calculate_current_time_ms(philo->start_time), philo->id + 1);
		}
		pthread_mutex_lock(philo->rules->mut_end);		
...
void	eat(t_philo *philo)
{
	...
	{
		if (check_death(philo) == 0 && read_end(philo) == 0 && philo->chopstick_taken == 0)
		{
		...
		if (check_death(philo) == 0 && read_end(philo) == 0 && philo->rules->number_of_philo > 1)
		{
		...
		if (check_death(philo) == 0 && read_end(philo) == 0 && philo->chopstick_taken == 2)
		{
...
{
    if (check_death(philo) == 0 && read_end(philo) == 0 && check_nb_meals(philo) == 0 && philo->rules->number_of_philo > 1)
    {
